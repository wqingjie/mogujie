...
~function () {
    ...
    ~function dataBind() {
        ...
        for (var i = 0; i < jsonData.length; i++) {
            var cur = jsonData[i];
            if (i === 0) {
                str += '<div class="show"><img src = ""trueSrc="' + cur.img + '"/></div>';
            } else {
    ...
    function autoMove() {
        ...
        if (step < oDiv.length - 1) {
            utils.setCss(oDiv[step + 1], "left", '178px');
            utils.setCss(oDiv[step + 1], "opacity", 0);
            utils.setCss(oDiv[step + 1], "z-index", 0);
            animate(oDiv[step], {left: -178, opacity: 0, "z-index": 0}, 600);
            animate(oDiv[step + 1], {left: 0, opacity: 1, "z-index": 3}, 600);
        }
        step++;
        if (step == oDiv.length) {
            step = 0;
            utils.setCss(oDiv[0], "left", '178px');
            utils.setCss(oDiv[0], "opacity", 0);
            utils.setCss(oDiv[0], "z-index", 0);
            animate(oDiv[oDiv.length - 1], {left: -178, opacity: 0, "z-index": 0}, 600);
            animate(oDiv[0], {left: 0, opacity: 1, "z-index": 3}, 600);

            return;
        }
        animate(oDiv[step - 1], {left: -178, opacity: 0, "z-index": 0}, 600);
        animate(oDiv[step], {left: 0, opacity: 1, "z-index": 3}, 600);
        align();
    }
...
~function () {
    function getTime() {
        var now = new Date();
        var target = new Date("2016/6/5 18:00:00");
